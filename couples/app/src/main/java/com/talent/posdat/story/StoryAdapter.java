    ...
    private void userInfo(final ViewHolder viewHolder, String userId, final int position)
    {
        ...
        reference.addListenerForSingleValueEvent(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                ...
                //check if  "Users" has all the attributes in Users class
                if (dataSnapshot.hasChild("profileImage")){
                    Picasso.get().load(user.getProfileImage()).placeholder(R.drawable.profile).into(viewHolder.story_photo);
                }

                if(position!=0)
                {
                    if (dataSnapshot.hasChild("profileImage")){
                        Picasso.get().load(user.getProfileImage()).placeholder(R.drawable.profile).into(viewHolder.story_photo_seen);
                    }
