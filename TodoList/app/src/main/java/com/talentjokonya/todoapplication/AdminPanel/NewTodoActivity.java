...
public class NewTodoActivity extends AppCompatActivity {
    ...
    @BindView(R.id.todo_crud_title_edt)
    EditText mTodoTitleEdt;
    @BindView(R.id.todo_crud_desription_edt)
    EditText mTodoDescriptionEdt;
    @BindView(R.id. todo_crud_urgentcheckbox)
    CheckBox mUrgentCheckBox;
    @BindView(R.id.togo_crud_deadline_edt)
    DateTimePickerEditText mDeadlineDateEdt;
    @BindView(R.id.todo_crud_addtodo_button)
        ...
        if (fAuth.getCurrentUser() != null) {
            ...
            if (isExist) {
                ...
                Thread mainThread = new Thread(new Runnable() {
                    @Override
                    public void run() {
                        newNoteRef.setValue(noteMap).addOnCompleteListener(task -> {
                            if (task.isSuccessful()) {
                                Toast.makeText(NewTodoActivity.this, "Note added to database", Toast.LENGTH_SHORT).show();
                            } else {
                                Toast.makeText(NewTodoActivity.this, "ERROR: " + task.getException().getMessage(), Toast.LENGTH_SHORT).show();
                            }
    ...
    private void deleteNote() {
        ...
        fNotesDatabase.child(noteID).removeValue().addOnCompleteListener(new OnCompleteListener<Void>() {
            ...
            public void onComplete(@NonNull Task<Void> task) {
                if (task.isSuccessful()) {
                    Toast.makeText(NewTodoActivity.this, "Note Deleted", Toast.LENGTH_SHORT).show();
                    noteID = "no";
                    finish();
                } else {
                    Log.e("NewTodoActivity", task.getException().toString());
                    Toast.makeText(NewTodoActivity.this, "ERROR: " + task.getException().getMessage(), Toast.LENGTH_SHORT).show();
                }
            }
        });
